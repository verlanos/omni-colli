from OmniUDPServer import OmniUDPServer
...
class OmniCollectServer(OmniComponent):
    def __init__(self, core_instance, address_to_listen_on):
        super(OmniCollectServer, self).__init__(core_instance)
        self.listening_socket = OmniUDPServer(core_instance, address_to_listen_on, UDPHandler)
...
class UDPHandler(SocketServer.BaseRequestHandler):
    def handle(self):
        ...
        event_obj = OmniEvent(device_id, payload)
        print("Triggering Event")
        self.server.getEventManager().fireEvent(event_obj)
